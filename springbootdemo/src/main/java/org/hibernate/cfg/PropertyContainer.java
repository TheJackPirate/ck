...
//    }
	...
	private void collectPersistentAttributesUsingLocalAccessType(
			LinkedHashMap<String,XProperty> persistentAttributeMap2,
		...
		while ( propertyIterator.hasNext() ) {
			...
			propertyIterator.remove();
			persistentAttributeMap2.put( xProperty.getName(), xProperty );
		}
		...
		while ( propertyIterator.hasNext() ) {
			...
			persistentAttributeMap2.put( name, xProperty );
	...
	private void collectPersistentAttributesUsingClassLevelAccessType(
		...
		if ( classLevelAccessType == AccessType.FIELD ) {
			for ( XProperty field : fields ) {
				if ( persistentAttributeMap2.containsKey( field.getName() ) ) {
					continue;
				}

				persistentAttributeMap2.put( field.getName(), field );
			}
		}
		else {
			for ( XProperty getter : getters ) {
				...
				if ( persistentAttributeMap2.containsKey( name ) ) {
					continue;
				}

				persistentAttributeMap2.put( getter.getName(), getter );
